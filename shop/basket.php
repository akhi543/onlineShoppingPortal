...
<head>
	<script>
		function removeProd(pid,bid) {
			...
			t.open("GET", "basketUtil.php?q="+pid+"&r="+bid,true);
			t.send();
		}
...
<?php
	...
	$r1 = mysqli_query($con, "select pname,quantity,pid,bid from basket natural join basketprods natural join product where  cid = $cid");
...
ab;
	for($i=1; $i<=$n; $i++) {
		...
		$bid = $an[3];
		echo "<tr><td>$i</td><td>$an[0]</td><td>$an[1]</td>";
		echo "<td><button onclick=\"removeProd($an[2],$an[3])\">Remove</button></td></tr>";
	}
	echo "</table>";
